...
E5_CL <- stats_by_group %>%
  filter(G_vec == "G1", Experiment == 5, Sampling_Method != "Complete_Data", BETA1 == 25, Model_Type == "propodds", Sample_Size %in% c(200, 500, 1500)) %>%
...
methods <- c("Best-SRS", "GA-Z-Bal", "GA-Y-Bal",
             "GA-YZ-Bal")
analyses <- c("Naive", "EM")
...
# Web Figure 25
p1 <- stats_by_group %>%
  filter(G_vec == "G1", Experiment == 5, Sampling_Method %in% c("Best-SRS", "GA-Z-Bal", "GA-Y-Bal", "GA-YZ-Bal"), BETA1 == 25) %>%
  ...
  labs(
    title = "Mean Absolute Relative Bias by Sample Size - Exp.5-B25",  
...
p3 <- stats_by_group %>%
  ...
  theme_bw ()  +
  facet_grid(Model_Type ~ Sampling_Method, labeller = labeller(Model_Type = model_type_labels))  + # , labeller = labeller(Sampling_Method = sampling_method_labels, Model_Type = model_type_labels)
  labs(
    title = "MSE Ratio - Exp.5-B25",  
    x = "Phase 2 Sample Size", 
    y = "MSE Ratio",  
    shape = "Analysis Method",  
    color = "Analysis Method",  
...
desired_order <- c("SRS", "Best-SRS", "Z-Bal", "GA-Z-Bal", "Y-Bal", "GA-Y-Bal", "YZ-Bal", "GA-YZ-Bal")

power_type1_error_results_0.05_all_tests <- power_type1_error_results_0.05_all_tests %>%
  mutate(Sampling_Method = factor(Sampling_Method, levels = desired_order))


# Web Figure 27

Power_Effect_E5_n500 <- power_type1_error_results_0.05_all_tests %>%
  filter(Test_Statistic == "LR", Experiment == 5, Sample_Size == 500, Analysis_Method != "Complete") %>%
  ...
  facet_grid(Model_Type ~ Sampling_Method, labeller = labeller(Model_Type = model_type_labels)) +
  labs(
    title = "Power vs Effect Size - Exp.5 (n=500)",
    x = "Effect Size",
    y = "Power",
    color = "Analysis Method",
...
Coverage_Effect_E5_n500 <- power_type1_error_results_0.05_all_tests %>%
  filter(Test_Statistic == "Wald", Experiment == 5, Sample_Size == 500, 
         Analysis_Method != "Complete", Sampling_Method %in% c("Best-SRS", "GA-Z-Bal", "GA-Y-Bal", "GA-YZ-Bal")) %>%
  ggplot(aes(x = factor(BETA1), y = Coverage)) +
  geom_point(aes(color = Analysis_Method), position = position_dodge(width = 0.7), size = 2) +  
  ...
  labs(
    title = "Coverage vs Effect Size - Exp.5 (n=500)",
    x = "Effect Size",
    y = "Coverage",
    color = "Analysis Method"
  ) +
  scale_x_discrete(labels = c("15" = "B15", "20" = "B20", "25" = "B25", "30" = "B30", "35" = "B35")) +  
  ylim(0.70, 0.98) +  # Set y-axis limits for coverage (0 to 1)
  theme_bw() +
  theme(
    axis.text.x = element_text(angle = 80, hjust = 1),
    legend.position = "bottom",
